...
class _QuestionGamePageState extends State<QuestionGamePage> {
              ...
              if (snapshot.data.data.status == 0) {
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        "We will add the questions soon. Keep checking the quiz.".tr,
                        textAlign: TextAlign.center,
                        style: textBody(),
                      ),
                      raisedBtnLg(
                        ...
                        children: [
                          ...
                          Column(
                            children: List.generate(
                                snapshot.data.data.questionVs.length, (index) {
                              isVs1.add(false);
                              isVs2.add(false);
                              isTie.add(false);

                              competitionListViewModel.vSQuestionId
                                  .add(snapshot.data.data.questionVs[index].id);
                              return _questionVs(
                                isBonus:
                                    snapshot.data.data.questionVs[index].bonus,
                                isTieOnTap: () {
                                  setState(() {
                                ...
                                v1Name:
                                    snapshot.data.data.questionVs[index].v1Name,
                                v2Name:
                                    snapshot.data.data.questionVs[index].v2Name,
                                v1ImgUrl: kImgBaseUrl +
                                    snapshot
                                        .data.data.questionVs[index].v1Image,
                                v2ImgUrl: kImgBaseUrl +
                                    snapshot
                                        .data.data.questionVs[index].v2Image,
                              );
                            }),
                          ),
                          Padding(
                              ...
                              child: Column(
                                children: List.generate(
                                  ...
                                  isopt3.add(false);
                                  isopt4.add(false);
                                  competitionListViewModel.mcqQuestionId.add(
                                      snapshot.data.data.questionMc[index].id);

                                  return _questionMc(
                                    isBonus: snapshot
                                        .data.data.questionMc[index].bonus,
                                    ...
                                    opt4Color: isopt4[index]
                                        ...
                                        : kColorAccent,
                                    option1: snapshot
                                        .data.data.questionMc[index].option1,
                                    option2: snapshot
                                        .data.data.questionMc[index].option2,
                                    option3: snapshot
                                        .data.data.questionMc[index].option3,
                                    option4: snapshot
                                        .data.data.questionMc[index].option4,
                                    onTapoption1: () {
                                      setState(() {
                                        if (competitionListViewModel.mcqAnswer
                                            .asMap()
                                            .containsKey(index)) {
                                    ...
                                    question: snapshot
                                        .data.data.questionMc[index].question,
                                  );
                                }),
                              )),
                          Column(
                              children: List.generate(
                                  ...
                                  (index) {
                            competitionListViewModel.tFQuestionId
                                .add(snapshot.data.data.questionTf[index].id);
                            isTrue.add(false);
                            isFalse.add(false);

                            return _questionTf(
                              isBonus:
                                  snapshot.data.data.questionTf[index].bonus,
                              trueOptColor:
                                  isTrue[index] ? Colors.white : kColorAccent,
                              falseOptColor:
                                  isFalse[index] ? Colors.white : kColorAccent,
                              onTapTrue: () {
                                setState(() {
                              ...
                              question:
                                  snapshot.data.data.questionTf[index].question,
                            );
                          })),
                              ...
                              ? Padding(
                                  ...
                                  child: CarouselSlider(
                                    items: List.generate(
                                      ...
                                      return InkWell(
                                        onTap: () async {
                                          if (await canLaunch(snapshot.data.data
                                              .competitionNativeAdd[0].url)) {
                                            ...
                                            await launch(snapshot.data.data
                                                .competitionNativeAdd[0].url);
                                          } else
                                    ...
                                    onTap: () async {
                                      ...
                                      if ((snapshot.data.data.expiredAt
                                        ...
                                        showAlertDialogMessge(
                                            context, "Quiz already expired".tr,
                                            () {
                                          Navigator.pop(context);
                                          NavRouter.navigator
                                              .pushNamed(NavRouter.mainPage);
                                        });
                                      } else {
                                        if (snapshot.data.data.questionVs.length == competitionListViewModel.vSAnswer.length &&
                                          ...
                                          if (competitionListViewModel
                                            ...
                                            if (snapshot.data.data.userMcqAnswer
                                                    .isNotEmpty ||
                                                snapshot.data.data.userTfAnswer
                                                    .isNotEmpty ||
                                                snapshot.data.data.userVsqAnswer
                                                    .isNotEmpty) {
                                              showAlertDialogMessge(
                                                  context,
  ...
  Widget _questionVs({
    ...
    return Column(
      children: [
        isBonus == 1 ? _bonusQuestion() : emptyWidget,
        Container(
          decoration: BoxDecoration(
